    ...
    advancedMonteCarlo(caseData, platform, iterations = 10000) {
        try {
            ...
            for (let i = 0; i < iterations; i++) {
                // Simular variações nas variáveis
    ...
    renderDashboard(containerId, caseData, platform) {
        ...
        try {
            ...
            container.innerHTML = `
                ...
                <style>
                    .quantum-extension-ext{ padding:0; } .risk-badge{ padding:8px 16px; border-radius:20px; font-size:0.7rem; font-weight:bold; } .risk-alto{ border-left:3px solid #ff1744; color:#ff1744; background:rgba(255,23,68,0.1); } .risk-moderado{ border-left:3px solid #ffc107; color:#ffc107; background:rgba(255,193,7,0.1); } .risk-baixo{ border-left:3px solid #00e676; color:#00e676; background:rgba(0,230,118,0.1); } .quantum-summary{ display:grid; grid-template-columns:repeat(4,1fr); gap:20px; margin-bottom:24px; } .sensitivity-grid{ display:grid; grid-template-columns:repeat(auto-fill,minmax(250px,1fr)); gap:16px; margin-bottom:24px; } .sensitivity-card{ background:var(--bg-terminal); border-radius:12px; padding:16px; } .montecarlo-grid{ display:grid; grid-template-columns:repeat(4,1fr); gap:16px; margin-bottom:20px; } .histogram-preview{ background:var(--bg-terminal); border-radius:12px; padding:16px; margin-top:16px; } .histogram-row{ display:flex; align-items:center; gap:12px; margin-bottom:8px; font-size:0.7rem; } .histogram-bar{ flex:1; height:20px; background:var(--bg-command); border-radius:10px; overflow:hidden; } .histogram-fill{ height:100%; background:linear-gradient(90deg,var(--elite-primary),var(--elite-success)); width:0; } .strategies-grid{ display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:16px; margin-bottom:20px; } .strategy-card{ background:var(--bg-terminal); border-radius:12px; padding:16px; } .strategy-card.optimal{ border:2px solid var(--elite-primary); background:rgba(0,229,255,0.05); } .pareto-front{ background:var(--bg-command); border-radius:8px; padding:12px; font-size:0.7rem; } .executive-summary{ margin-top:24px; } .summary-card-exec{ background:linear-gradient(135deg, rgba(0,229,255,0.05), transparent); border:1px solid var(--elite-primary); border-radius:16px; padding:20px; } .exec-headline{ font-size:1.1rem; font-weight:bold; color:var(--elite-primary); margin-bottom:12px; } @media (max-width:768px){ .quantum-summary{ grid-template-columns:1fr 1fr; } .montecarlo-grid{ grid-template-columns:1fr 1fr; } }
                </style>
            `;
        } catch (error) {
