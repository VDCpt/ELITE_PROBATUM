...
// Interceptação de requisições - estratégia Cache First com fallback para rede
self.addEventListener('fetch', (event) => {
    const url = new URL(event.request.url);
    
    // Requisições de API - usar Network First
...
/**
 * Estratégia Stale While Revalidate - serve cache, atualiza em background
 */
...
/**
 * Cache de artefactos de caso (Digital Briefcase)
 * Chamado pelo módulo Strategic Vault ao exportar pacote forense
 */
...
async function cacheCaseArtefacts(caseId, artefacts) {
    ...
    // Cache dos certificados
    for (const cert of artefacts.certificates) {
        const certKey = `case_${caseId}_cert_${cert.id}`;
        await cache.put(certKey, new Response(JSON.stringify(cert), {
            headers: { 'Content-Type': 'application/json' }
        }));
    }
    
    console.log(`[ServiceWorker] Artefactos do caso ${caseId} cacheados com sucesso`);
...
async function getCachedCases(event) {
    ...
    event.ports[0].postMessage(Array.from(cases));
}
...
// ============================================================================
// BACKGROUND SYNC PARA SUBMISSÕES PENDENTES
// ============================================================================

let pendingSubmissions = [];

/**
 * Registra submissão pendente para sincronização em background
 */
...
/**
 * Sincroniza submissões pendentes com o servidor
 */
...
/**
 * Obtém submissões pendentes do IndexedDB
 */
...
/**
 * Remove submissão pendente após sincronização
 */
...
// ============================================================================
// NOTIFICAÇÕES PUSH PARA PRAZOS JUDICIAIS
// ============================================================================
...
self.addEventListener('push', (event) => {
    ...
    if (event.data) {
        try {
            ...
            data = { title: 'ELITE PROBATUM', body: event.data.text() };
        }
    }
...
// ============================================================================
// UTILITÁRIOS PARA CLIENTE
// ============================================================================

/**
 * Expõe API para o cliente através de mensagens
 */
